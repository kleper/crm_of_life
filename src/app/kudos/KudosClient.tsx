    ...
    <div className="flex flex-col gap-6 h-full">
      ...
      <div className="bg-white border border-slate-200 p-5 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          ...
          <Button onClick={() => setIsModalOpen(true)}>+ Enviar Kudo</Button>
        </div>
      ...
      <div className="flex gap-2 border-b border-slate-200">
        <button 
          onClick={() => setTab("wall")}
          className={`px-4 py-2 text-sm font-bold uppercase transition-colors ${tab === "wall" ? "border-b-2 border-indigo-600 text-indigo-700" : "text-slate-500 hover:text-slate-800"}`}
        >
          Muro del Equipo
        </button>
        <button 
          onClick={() => setTab("received")}
          className={`px-4 py-2 text-sm font-bold uppercase transition-colors ${tab === "received" ? "border-b-2 border-indigo-600 text-indigo-700" : "text-slate-500 hover:text-slate-800"}`}
        >
          Recibidos
        </button>
        <button 
          onClick={() => setTab("sent")}
          className={`px-4 py-2 text-sm font-bold uppercase transition-colors ${tab === "sent" ? "border-b-2 border-indigo-600 text-indigo-700" : "text-slate-500 hover:text-slate-800"}`}
        >
          Enviados ({summary.totalSent}/3 hoy)
        </button>
      </div>
