          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {tenantUsers.map((tu, idx) => (
                <tr key={tu.id} className={`border-b border-slate-100 transition-colors hover:bg-amber-50 ${idx % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}`}>
                  <td className="p-4 text-slate-700 font-bold">{tu.user.name || "Sin nombre"}</td>
                  <td className="p-4 text-slate-600 font-medium">{tu.user.email}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-slate-500 font-medium">{new Date(tu.createdAt).toLocaleDateString()}</td>
                </tr>
              ))}
              {tenantUsers.length === 0 && (
                <tr>
                  <td colSpan={4} className="p-8 text-center text-slate-500 font-medium bg-slate-50">No hay usuarios en esta organización.</td>
                </tr>
              )}
            </tbody>
          </table>
